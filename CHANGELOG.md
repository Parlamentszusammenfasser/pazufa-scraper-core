...
- **The HTML-to-plain-text conversion moved into new file 'html_text.py'**, again to shorten 'text.py': the element name sets, the markup patterns and their helpers now live there behind `html_to_text()`, which `normalize_volltext` calls as step 0. Behaviour is unchanged; the module is named `html_text` so it cannot be confused with the standard library's `html`, which `text.py` uses for entities. `text.py` is 236 lines again instead of 441.
